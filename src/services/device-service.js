...
// TODO : implement
function isAirDevice(deviceProfile) {
    return true;
}
 ...
 * 
 * @param {*} device 
 * @returns one of 'AIR_PURIFIER', 'AIR_MONITOR', 'CONTACT_SENSOR', 'OTHERS'
 */
function getDeviceType(deviceProfile) {
    // Ait Monitor by device profile ID
    if (deviceProfile?.profile?.id == '17d6783f-67a1-4600-a85b-40f7c0d87084'
        || deviceProfile?.profile?.id == '1bcc08ee-95ef-4eef-957e-c99b6e17962f') {
        return 'AIR_MONITOR';
    } else if (deviceProfile?.profile?.id == '247bfdeb-3358-4d42-b25f-5710bfb4a839'
        || deviceProfile?.profile?.id == '9ec557b3-768e-3b9b-8ced-a2760b1afd8f') {
        return 'AIR_PURIFIER';
    }
    return 'OTHERS';
}
...
function hasMainSwitch(deviceProfile) {
    if (deviceProfile?.components) {
        const filtered = deviceProfile.components
            .filter((component) => component.id == 'main'
                && component.capabilities.filter((capability) => capability.id == 'switch').length > 0);
...
module.exports = class DeviceService {
    ...
    async getAirDevices() {
        ...
        const filteredDevices = deviceProfiles
            .filter((deviceProfile) => isAirDevice(deviceProfile));
